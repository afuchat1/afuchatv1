...
const Search = () => {
  ...
  const handleSearch = useCallback(async () => {
    ...
    try {
      ...
      const { data: webSearchData, error: webError } = await supabase.functions.invoke('you-search', {
  ...
  const hasAnyResults = results.length > 0 || webResults.length > 0;
      ...
      <div className="flex-1 overflow-y-auto p-4">
        {loading && isSearchActive ? (
          ...
          <div className="space-y-6">
            ...
            {webResults.length > 0 && (
              <section>
                <h2 className="text-sm font-bold text-foreground mb-3 border-b pb-1 flex items-center gap-2">
            ...
            {webLoading && (
              <div className="flex items-center justify-center py-8">
                <CustomLoader size="sm" text="Searching the web..." />
              </div>
            )}
          </div>
        )}
